...
<section class="bg-primary text-white">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 py-16 text-center">
        <h1 class="text-3xl md:text-5xl font-bold mb-4">{{ $store->name }}</h1>
        ...
        <form action="{{ route('products.search') }}" method="GET" class="max-w-xl mx-auto">
            <div class="flex gap-2">
                <input type="search" name="q" placeholder="Search for products..." required
                       class="flex-1 px-5 py-3 rounded-full text-gray-900 focus:outline-none text-sm">
                <button type="submit" class="bg-white text-primary font-bold px-6 py-3 rounded-full text-sm hover:bg-gray-100">
                    Search
                </button>
            </div>
        </form>
...
<section class="max-w-7xl mx-auto px-4 sm:px-6 py-10">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
        ...
        <a href="{{ route('products.category', $cat) }}"
           class="bg-white rounded-xl p-4 text-center shadow-sm hover:shadow-md transition-shadow border hover:border-primary group">
...
{{-- Trust badges --}}
<section class="bg-white border-t border-b mt-8">
